# v1.1.3 Steam Windows 商業發行檢查表

## 自動化門檻

- [x] 283 筆內容通過 JSON Schema、穩定 ID 與跨檔引用驗證。
- [x] 11 個發行圖片／圖示逐檔存在、授權非空且 SHA-256 完全相符；10 張 Runtime 圖片另通過解碼、真 alpha、原稿雜湊、前景零遺失、圖集邊界、白色內容保留及 42 張實機證據檢查，共 712 項。
...
- [x] 共同、私人、競賽農場及 1／2／3／5／16 人劇情拓撲以 7 台專用伺服器＋31 個客戶端實連，通過版本握手、權威移動、16 人最大容量、農場整合／隔離、四種篇章、戀愛平手／領先／唯一婚約與原子世界存檔。
...
- [x] Steam 候選版 51/51：Big Picture 全螢幕、兩個啟動項、功能勾選契約、18 組鍵盤／XInput 映射、0 支手把的證據限制及 1280×800 最小 30px 有效字級通過。
- [x] v1.1.3 正式 EXE 以 `SteamTenfoot=1` 啟動，實際 Windows 視窗為 1920×1082 並完整覆蓋 1920×1080 螢幕，正常以 exit code 0 自動結束。
...
- Steam 手把宣稱：只標部分控制器支援；沒有手把仍可發布 Windows 鍵盤滑鼠版，但不可宣稱完整控制器支援或 Deck Verified
- 外部未結案：受信任的 Authenticode 憑證、正式法律意見，以及 Steam 帳號／App ID／Depot／問卷／商店頁／Valve review
