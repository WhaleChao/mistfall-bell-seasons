...
async def assist_events(
    ...
    for attempt in range(3):
        if attempt:
            yield {"type": "warning", "message": f"結構驗證失敗，正在進行第 {attempt} 次自動修復。"}
            prompt += (
                "\n\n上一版輸出不合法。錯誤如下：\n- " + "\n- ".join(last_errors[:12]) +
                "\n請完整重輸出修正後的 JSON，不要解釋。"
            )
        chunks: list[str] = []
        try:
            async for token in ollama.stream_chat(model, SYSTEM_PROMPT, prompt, schema or "json", image_paths):
                chunks.append(token)
        ...
        if result.valid:
            ...
            yield {"type": "done", "model": model, "source_count": len(sources)}
            return
