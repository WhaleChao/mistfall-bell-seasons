...
結果：**PASS**　｜　157 通過／0 失敗　｜　23.32 秒　｜　Apple M4
...
| 角色建立 | 玩家名稱欄位 | 通過 |  |
| 角色建立 | 四種外觀選項 | 通過 |  |
| 輸入焦點 | 首啟自動聚焦玩家名稱欄位 | 通過 |  |
| 輸入焦點 | 名稱輸入 E 不會誤觸互動或開始遊戲 | 通過 |  |
| 輸入焦點 | 名稱輸入 M 不會誤開連線介面 | 通過 |  |
| 標題輸入 | 標題只接受 ui_accept 開始 | 通過 |  |
| 角色建立 | 套用名稱與外觀 | 通過 |  |
| 啟動 | 離開標題後恢復遊戲 | 通過 |  |
| 輸入焦點 | 任意 LineEdit 編輯時忽略遊戲全域快捷鍵 | 通過 |  |
| 移動動畫 | ui_right 方向移動 | 通過 | 位移 79.9 px |
| 移動動畫 | ui_right 使用正確方向列 | 通過 | atlas row=2 |
| 移動動畫 | ui_right 播放多幀循環 | 通過 | 觀察到 3/4 幀 |
...
| 節慶 | 五回合評分與獎勵 | 通過 |  |
| 節慶 | 四季 12 場節慶資料 | 通過 |  |
| 主線互動 | inactive 與已看過章節可重談 | 通過 |  |
| 主線互動 | active 未達標不重播章節或發獎 | 通過 |  |
| 主線互動 | 既有 seen＋active 存檔以真 E 結算 | 通過 |  |
| 主線互動 | 成功只發獎一次並開放下一章 | 通過 |  |
| 選單 | 旅人手冊暫停遊戲 | 通過 |  |
| 選單 | 十一個功能分頁 | 通過 |  |
