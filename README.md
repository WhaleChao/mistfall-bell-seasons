...
- 畜舍內可操作的 6×4 鐘網設備；電力／水量網路可自動播種、澆水、收割、餵食、加工霧封農產與凝析夢潮鹽，支援開關、優先序、作物篩選及跨存檔保存，不再藏在設定頁。
...
- 農場、村莊、河畔、古鐘林、古代都市、鐘窟、夢岸與農舍／畜舍／溫室共 10 張獨立 MapScene；道路、橋梁、山口、門與洞口都有雙向連線及可達性驗證。
...
- 四方向站立／四幀走路、三階段攻擊與同方向命中／刀光；角色以腳底為錨點並使用最終尺寸點陣素材，人物與門、橋、床及機器的比例由自動測試鎖定。
- 194 個種子、作物、魚、蛋、藥水、藥草、齒輪、礦石、工具、料理、畜產品與設備視覺 ID 都有獨立 PNG；缺圖會直接使測試失敗。
- SaveGame v6；可逐版遷移 v1（28 日制）至 v5，另保存地圖、傳送門、位置、面向、室內狀態與動物所在場景。
...
發行閘門會驗證 299 筆內容、JSON Schema／引用、資產 SHA-256／授權、194 個世界物件視覺 ID、Save v1→v2→v3→v4→v5→v6、12,000 日／100 年、12,000 個自動化週期、400 次自動化月存檔、250 次磁碟存讀、20 敵人效能、五種解析度、PCK 邊界與正式 ZIP。[全地圖重製報告](reports/world_v2/REPORT.md)另保存 10 張固定截圖、物理可達性、傳送圖、角色比例、100 次建物進出及目標 Mac 五分鐘戰鬥結果。既有[全功能實機驗收報告](reports/full_feature_acceptance/REPORT.md)與[圖片閘門](reports/image_integrity/REPORT.md)仍保留作歷史基準；[Steam 品質基準](reports/steam_candidate/REPORT.md)只作檢查、不執行上傳。正式版預設離線。
